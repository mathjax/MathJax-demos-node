...
const Typeset = exports.Typeset = {
  ...
  create: {
    ...
    input(args) {
      ...
      const jax = Util.import(`@mathjax/src/js/input/${name}.js`)[Typeset.jax[args.input]];
    ...
    output(args) {
      ...
      const jax = Util.import(`@mathjax/src/js/output/${name}.js`)[Typeset.jax[name]];
  ...
  hooks: {
    ...
    args(args) {
      ...
      //
      if (Util.mixed) {
        const {Loader} = Util.import('@mathjax/src/js/components/loader.js');
        Util.import(`@mathjax/src/components/js/input/${args.input}/${args.input}.js`);
        Loader.preLoaded(`input/${args.input}`);
        if (args.output !== 'mml') {
          Util.import(`@mathjax/src/components/js/output/${args.output}/${args.output}.js`);
