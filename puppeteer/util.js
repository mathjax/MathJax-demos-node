Util = {
  /**
   * The scripts that are originally in the document.
   */
  scripts: Array.from(document.querySelectorAll('script')).filter(name => name !== document.currentScript),

  /**
   * Replace the MathDocument's renderPromise() method with one that
   * calls the original and then removes any scripts inserted by MathJax.
   */
  removeScripts() {
    Object.assign(MathJax.startup.document, {
      _renderPromise: MathJax.startup.document.renderPromise,
      async renderPromise() {
        await this._renderPromise();
        document.querySelectorAll('script').forEach((script) => {
          if (!Util.scripts.includes(script)) script.remove();
        });
      }
    });
  },
  ...
  /**
   * Functions from the Chtml.js utility.
   */
  chtml: {
    /**
  ...
  /**
   * Functions from the Svg.js utility.
   */
  svg: {
    /**
    ...
    filterPage(args, html = MathJax.startup.document) {
      ...
      if (list.length === 0) {
        ...
        const cache = adaptor.getElements(adaptor.body(html.document), '#MJX-SVG-global-cache')[0];
  ...
  /**
   * Functions from the Mml.js utility.
   */
  mml: {
    input: {
      /**
    ...
    /*
     * Functions from the Util.js utility.
     */
  ...
  /**
   * Run the input and ouptut ready functions, if any.
   */
  startup(args) {
    for (const jax of ['input', 'output']) {
      const ready = (Util[args[jax]]?.[jax] || Util[args[jax]])?.ready;
      if (ready) {
        ready(args, MathJax.startup.document);
      }
    }
  },
 ...
 */
process = {
  exit() {console.error('_die_'); throw "Exiting..."}
}

/**
 * Make these Util libraries available.
 */
Mml = Util.mml;
Svg = Util.svg;
Am = {};
